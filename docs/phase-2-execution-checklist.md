...
| P2-016 | Complete | Add CardDetail comps panel only after comps artifact exists | Claude/Codex | Specific `src/` files named by user | `npm run build`; `node scripts/verify-data.js` | Comps panel separates raw/graded, variants, confidence, and outliers. | Sample-gated CardDetail panel complete. Production path `/ebayCompsSummary.json` only; sample artifacts refused. |
...
| 2026-05-12 | P2-015 | `0110c23` | Added sample-gated premium metadata loader, prop-driven premium badge component, CardDetail badge rendering, and Value Scanner micro badges. | `npm run build` (✓ 653.53 kB raw / 97.09 kB gzip); `node scripts/verify-data.js` (✓ 9 invariants) | Production UI fetches `/premiumMetadata.json`; sample artifacts are refused and current UI renders no premium badges until a production artifact exists. |
| 2026-05-12 | P2-016 | Current UI commit | Added sample-gated eBay comps loader, CardDetail comps panel, raw/graded separation, on-demand aggregates, variant chips, and insufficient-comps gate. | `npm run build`; `node scripts/verify-data.js` | Production UI fetches `/ebayCompsSummary.json`; sample artifacts are refused and current panel renders the awaiting-fixture state until a production artifact exists. |
...
Next recommended task: **`P2-017` (Backend decision) — operator-only gate**.
Backend work remains unapproved until at least one Backend Trigger Checklist
condition in § 7 is true and the operator explicitly approves the task.
