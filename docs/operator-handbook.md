...
## 4c. Ready-to-paste prompt: Vite 8 upgrade (Codex) — closes R-055

**Operator gate:** This is a breaking major version bump (Vite 5
→ 8). The advisory (R-055 — esbuild dev-server) affects dev-server
only; production is unaffected. So this is "good operational
hygiene," not urgent. Only paste when you have ~30 minutes to
review Codex's report carefully — if the build/test outputs look
funky, you might need to revert.

Bounded scope: one Codex run, one commit, full smoke test. If any
gate fails, Codex stops and reports without committing.

```text
Upgrade FusionMetrics from Vite 5.4.1 to the latest Vite 8.x.
Closes R-055 (esbuild dev-server advisory) by carrying the
esbuild transitive forward to a non-vulnerable version. One
commit. Operator pre-approved.

Read first:
- AGENTS.md (especially § 5 commands and § 6 file boundaries)
- CLAUDE.md § 7.7 (dependency policy — Vite is an existing dep
  being upgraded, not a new one; this is bumping a pinned
  version)
- docs/risk-register.md R-055
- vite.config.js
- vitest.config.js
- package.json + package-lock.json
- The Vite migration guide at https://vite.dev for the most
  recent major. Note any required config changes.
...
Preflight:
  ...
  node scripts/verify-data.js
  npm test
Confirm: clean tree, 9 invariants, 20 tests pass on the current
Vite 5.4.1 baseline.

────────────────────────────────────────────────────────────────────
SINGLE COMMIT — chore: Vite 5 → 8 upgrade
────────────────────────────────────────────────────────────────────

Allowed files:
- package.json (EDIT — bump `vite` to ^8.x in devDependencies;
  bump `@vitejs/plugin-react` if its version constraint requires
  a matching update)
- package-lock.json (auto-regenerated by `npm install`)
- vite.config.js (EDIT only if migration requires it; document
  every line changed in the commit body)
- vitest.config.js (EDIT only if migration requires it)
- docs/risk-register.md (EDIT — flip R-055 to mitigated/closed;
  add change log row)
- docs/phase-3-execution-checklist.md (EDIT — add a ledger row
  for the upgrade)

Forbidden: every other file. ESPECIALLY: src/*, scripts/*,
public/*, all generated artifacts, .github/workflows/*.

Validation gates (every gate must pass; stop and report on first
failure):
  1. npm install                  (lockfile updated cleanly)
  2. npm ci                       (lockfile in sync)
  3. node scripts/verify-data.js  (9 invariants)
  4. npm run build                (succeeds)
  5. Bundle delta < 5 kB raw vs prior 662 kB                  ←
     CRITICAL. Vite majors can regress chunking.
  6. npm test                     (all 20 tests still pass)
  7. npm audit                    (the two moderate esbuild
                                   advisories no longer present)
  8. npm run dev                  (server starts; manual ctrl-C
                                   after confirming it boots)

Commit message body must include:
- Old Vite version → new Vite version
- Bundle delta (must report exact bytes)
- Test suite result
- Any config-file changes with one-line rationale per change
- npm audit before/after summary

Final response:
1. Commit SHA + subject.
2. Bundle delta exact bytes.
3. Test results.
4. Config changes (line count, files touched).
5. npm audit before/after.
6. STOP. Do not push. Operator pushes after reviewing.

STOP CONDITIONS:
- Any test FAILS.
- Bundle grows > 5 kB raw.
- Any non-config file appears in diff.
- New audit advisory appears (other than R-055 closing).
- Vite migration guide flags a breaking change that needs
  src/ edits — STOP. Out of scope. Report and ask.
- `npm install` brings in unexpected transitive bloat
  (> 30 MB node_modules growth) — STOP and report.
```

---

## 4d. Ready-to-paste prompt: ship Set Rankings + Chase Radar (Codex) — operator-gated

**Operator gate:** This implements the spec at
`docs/set-rankings-spec.md`. Two new tabs in the dashboard. The
spec is already authored (P3-010); implementation just needs your
greenlight.

Paste when you've reviewed `docs/set-rankings-spec.md` and want it
built:

```text
Implement FusionMetrics Set Rankings + Chase Radar surfaces per
docs/set-rankings-spec.md. Operator pre-approved. Two new tabs in
the dashboard; pure-function aggregates over existing data.js
exports; no new dependencies; no generated-artifact edits.

Read first:
- docs/set-rankings-spec.md (the canonical spec — follow every §)
- AGENTS.md (forbidden language, file boundaries)
- docs/decision-log.md D-006, D-009, D-014, D-043
- src/data.js (existing exports: CARDS, SETS, RARITIES,
  HAS_LIVE_PRICES)
- src/App.jsx (current tab system)
- src/tabs/MarketDynamics.jsx (existing set-level surface;
  reference for layout)
- src/components/CardRow.jsx (reused by Chase Radar)
- docs/test-coverage-gap-analysis.md (test patterns)

Preflight:
  bash scripts/session-brief.sh
  git fetch --all && git pull --ff-only
  npm test    (must show 20 passing on the existing suite)

────────────────────────────────────────────────────────────────────
COMMIT 1 — feat: set-aggregates pure functions + tests
────────────────────────────────────────────────────────────────────

Allowed files:
- src/lib/setAggregates.js (NEW — exports computeSetAggregates,
  computeChaseRadar; pure functions taking CARDS array)
- tests/setAggregates.test.js (NEW — at least 4 cases per spec § 10)
- vitest.config.js (NO edit unless config requires it)

Validation: npm test (24 passing); verify-data.

────────────────────────────────────────────────────────────────────
COMMIT 2 — feat: Set Rankings tab
────────────────────────────────────────────────────────────────────

Allowed files:
- src/tabs/SetRankings.jsx (NEW — consumes setAggregates; mobile
  per spec § 7; coverage-status chip per § 5)
- src/App.jsx (EDIT — add tab entry between Market Dynamics
  and Box EV)
- tests/setRankings.test.jsx (NEW — at least 2 RTL cases:
  empty state and coverage chip rendering)

Validation: npm run build (< 30 kB bundle growth); npm test; verify-data.

────────────────────────────────────────────────────────────────────
COMMIT 3 — feat: Chase Radar tab
────────────────────────────────────────────────────────────────────

Allowed files:
- src/tabs/ChaseRadar.jsx (NEW — reuses CardRow; sort + filter
  per spec § 4)
- src/App.jsx (EDIT — add tab entry after Watchlist)
- tests/chaseRadar.test.jsx (NEW — at least 2 RTL cases:
  default sort and filter interaction)
- docs/phase-3-execution-checklist.md (EDIT — flip P3-010 to
  Complete, ledger row)

Validation: npm run build (cumulative bundle growth < 30 kB
across both new tabs); npm test (28 passing); verify-data.

Forbidden in all three commits: src/data.js, all generated
artifacts, scripts/*, .github/workflows/*, package files.

Forbidden language enforcement: scan every new copy string
against `tests/setRankings.test.jsx` regex case before commit.

Final response: 3 SHAs, bundle deltas per commit, test summary.
STOP. Do not push.
```
